    ...
    OwnableUpgradeable
{
    ...
    function initialize(
        ...
        require
        (
            factory_ != address(0) && router_ != address(0) && wsei_ != address(0)
            && dragonswapFactory_ != address(0) && dragonswapRouter_ != address(0),
    ...
    function _launch(
    ...
    ) private returns (address, address, uint) {
        // 1. deploy new token
        ...
        // 3. ***NEW*** — use virtual‑inclusive reserves for launch metrics
        (uint256 resToken, uint256 resAsset) = IFPair(_pair).getReserves();
        ...
        Data memory _data = Data({
            ...
            ticker: _ticker,
            supply: supply,
            price: resToken / resAsset,
            marketCap: (supply * resAsset) / resToken,
            liquidity: resAsset * 2,
            volume: 0,
            volume24H: 0,
            prevPrice: resToken / resAsset,
...
/// HELPER QUERY FUNCTIONS ///
    ...
    function getMaxLaunchInput(uint256 launchFee) private view returns (uint256 maxInput) {
        // The protocol caps the first buy at `maxTx` percent of **real** supply.
        // Virtual-token cushion (vT) is ignored here; it only smooths the curve,
        // it does not loosen the concentration limit.
        uint256 syntheticAssets = factory.defaultVirtSei(); // vA

        uint256 maxBuy = maxBuyInput(
            initialSupply,  // real supply
            maxTx,
            initialSupply,  // tokenBalance in pool at launch (real)
            syntheticAssets // virtual SEI cushion used for price math
        );
        maxInput = maxBuy + launchFee; // add back flat fee
    }
    ...
    function getMaxBuyInputAsset(address token, address assetToken)
        public view
        returns (uint256 maxInput)
    {
        ...
        FERC20   tkn     = FERC20(token);
        uint256  supply  = tkn.totalSupply();
        uint256  maxPct  = tkn.maxTx();

        // pricing reserves **with** virtual liquidity
        (uint256 tokenRes, uint256 assetRes) = pair.getReserves();

        return maxBuyInput(supply, maxPct, tokenRes, assetRes);
    }
    ...
    function maxBuyInput(
        ...
        uint256 hardCap = (totalSupply * tokenMaxTx) / 100;  // 10 % supply
        uint256 buyAmount;

        if (tokenBalance <= hardCap) {
            buyAmount = tokenBalance;                        // flat-curve zone
        } else {
            buyAmount = (hardCap * assetBalance) / (tokenBalance - hardCap);
        }

        // gross-up for router buy-tax
        uint256 buyTax = factory.buyTax();                   // pull once
        uint256 buyAmountWithTax = buyAmount * 100 / (100 - buyTax);

        maxBuy = buyAmountWithTax;                           // assign to return var
    }
